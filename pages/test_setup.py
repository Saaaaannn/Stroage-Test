...
class TestSetupPage(ttk.Frame):
    def __init__(self, parent, controller):
        ...
        self.selected_tests = {}  # 存储测试项选择状态
        self.is_testing = False  # 防止重复启动
    ...
    def run_all_tests(self, usb_info, selected_names):
        ...
        try:
            ...
            for name in selected_names:
                ...
                if cls:
                    self.logger.log_message(f"--- 开始: {name} ---", "INFO")
                    try:
                        test = cls(usb_info, self.logger)
                        if test.run():
                            self.logger.log_message(f"✅ {name} 通过", "INFO")
                        else:
                            self.logger.log_message(f"❌ {name} 失败", "ERROR")
                            all_passed = False
                    except Exception as e:
                        self.logger.log_message(f"❌ {name} 执行异常: {e}", "ERROR")
                        all_passed = False

            final = "🎉 所有测试通过！" if all_passed else "⚠️ 部分测试失败"
            self.logger.log_message("-" * 50, "INFO")
            self.logger.log_message(f"测试完成: {final}", "INFO")
